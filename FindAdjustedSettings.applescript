...
if resultsInCollections then
    ...
    tell application "Capture One 21" to copy ref2ResultAlbum to ref2NotAdjustedAlbum
    InitializeResultsCollection(Result_ProjectName, "IsAdjusted")
    tell application "Capture One 21" to copy ref2ResultAlbum to ref2IsAdjustedAlbum
end if
...
repeat with theVariant in theVariantList
    tell application "Capture One 21" to copy adjustments theVariant -- Step 1 of 2 do this early to give C1 time to react
    ...
    tell application "Capture One 21" to tell parent image of theVariant to set {theCameraModel, theEXIFISO_T} to get {EXIF camera model, EXIF ISO}
    ...
    tell application "Capture One 21" to set {{theCropHorCenter, theCropVerCenter, theCropWidth, theCropHeight}, theLCCName, countStyles, countLayers, {theImageWidth, theImageHeight}} to ¬
    ...
    if not (0 = countStyles) then
        ...
        if logEveryAdjustment then
            tell application "Capture One 21" to set theStyleList to get styles of theVariant
    ...
    tell application "Capture One 21" to tell adjustments of theVariant to set theProperties to get properties
    ...
    if not variantIsAdjusted then
        ...
        if resultsInCollections then tell application "Capture One 21" to tell currentDocRef to add inside ref2NotAdjustedAlbum variants {theVariant}
        ...
        if resultsInCollections then tell application "Capture One 21" to tell currentDocRef to add inside ref2IsAdjustedAlbum variants {theVariant}
    ...
    if AdjustmentValuesInMetadata then
        ...
        tell application "Capture One 21" to tell theVariant to ¬
...
tell application "Capture One 21" to select currentDocRef variants theSelectedVariantList
...
on getVariantNameNum(theVariant)
    tell application "Capture One 21" to tell theVariant to set {theVariantPosition, theBaseName} to {position, name}
    tell application "Capture One 21" to tell (get parent image of theVariant) to set theParentsVariantCount to (count of variants)
...
on getVariantsImageType(theVariant)
    tell application "Capture One 21" to tell theVariant to set theFileExt to extension of parent image
    return theFileExt
...
on InitializeResultsCollection(nameResultProject, nameResultAlbumRoot)
    ...
    tell application "Capture One 21" to tell currentDocRef
